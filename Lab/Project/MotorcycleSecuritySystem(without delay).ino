...
void manageStateChangeBlink();
void manageTestAlarm();
void startStateChangeBlink(bool armedState);
void handleButtonPress(byte buttonCode, byte buttonIndex);
void waitForMillisWithGsmDrain(unsigned long durationMs);
...
// ===== Non-Blocking State Blink Variables =====
bool stateBlinkInProgress = false;
bool stateBlinkIsArmed = false;
unsigned long stateBlinkLastToggleTime = 0;
byte stateBlinkToggleCount = 0;

// ===== Button Cooldown Variables =====
unsigned long buttonCooldownUntil[3] = {0, 0, 0};
const unsigned long BUTTON_COOLDOWN_MS = 300;

// ===== Test Alarm Variables =====
bool testAlarmInProgress = false;
byte testAlarmPhase = 0;
unsigned long testAlarmPhaseStartTime = 0;
...
void checkButtons() {
  ...
  if (digitalRead(BUTTON1_PIN) == LOW) {
    handleButtonPress(BUTTON1_CODE, 0);
  }

  // Button 2: Sequence input
  if (digitalRead(BUTTON2_PIN) == LOW) {
    handleButtonPress(BUTTON2_CODE, 1);
  }

  // Button 3: Sequence input
  if (digitalRead(BUTTON3_PIN) == LOW) {
    handleButtonPress(BUTTON3_CODE, 2);
  }
}
...
void handleButtonPress(byte buttonCode, byte buttonIndex) {
  ...
  if (now < buttonCooldownUntil[buttonIndex]) {
    return;
  }

  buttonCooldownUntil[buttonIndex] = now + BUTTON_COOLDOWN_MS;

  if (inLockout) {
    // In lockout: part of secret sequence
    addToSecretSequence(buttonCode);
  }

  if (!inLockout) {
    addToAdminSequence(buttonCode);
  }
}
...
void checkSecretSequence() {
  ...
  if (sequenceCorrect) {
    ...
    // Confirm with non-blocking beeps
...
void testAlarm() {
  ...
  testAlarmInProgress = true;
  testAlarmPhase = 1;
  testAlarmPhaseStartTime = millis();
  inStage1 = true;
  inStage2 = false;
}
...
void setState(SystemState newState) {
  if (newState != currentState) {
    ...
    if (newState == ARMED) {
      startStateChangeBlink(true);
    } else if (newState == DISARMED) {
      startStateChangeBlink(false);
...
void indicateLogsOnGreenLed() {
  ...
  stateBlinkInProgress = false;
  stateBlinkToggleCount = 0;
  stateBlinkLastToggleTime = millis();
}
...
Serial.println(F("[GSM] SMS..."));
  ...
  waitForMillisWithGsmDrain(500); 
  
  gsmSerial.print(F("AT+CMGS=\""));
  gsmSerial.print(MASTER_PHONE);
  gsmSerial.println(F("\""));
  waitForMillisWithGsmDrain(1000); 
  ...
  waitForMillisWithGsmDrain(4000);
  gpsSerial.listen();
}
...
void startStateChangeBlink(bool armedState) {
  stateBlinkInProgress = true;
  stateBlinkIsArmed = armedState;
  stateBlinkLastToggleTime = millis();
  stateBlinkToggleCount = 0;
...
void manageStateChangeBlink() {
  ...
  if ((now - stateBlinkLastToggleTime) < blinkInterval) {
    return;
  }

  stateBlinkLastToggleTime = now;
  byte targetPin = stateBlinkIsArmed ? RED_LED_PIN : GREEN_LED_PIN;
  bool currentLevel = digitalRead(targetPin);
  digitalWrite(targetPin, !currentLevel);

  if (currentLevel == HIGH) {
    stateBlinkToggleCount++;
    if (stateBlinkToggleCount >= 2) {
...
void manageTestAlarm() {
  ...
  if (testAlarmPhase == 1) {
    if ((now - testAlarmPhaseStartTime) >= STAGE1_DURATION) {
      Serial.println(F("Testing Stage 2 (3 seconds)..."));
      testAlarmPhase = 2;
      testAlarmPhaseStartTime = now;
      inStage1 = false;
      inStage2 = true;
    } else {
